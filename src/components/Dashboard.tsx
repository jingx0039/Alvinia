...
export default function Dashboard() {
  ...
  const fetchContacts = async () => {
    ...
    setErrorString(null);
    try {
      // First, get system configuration health
      ...
      // We successfully loaded *some* data (could be memory fallback). Check config error.
      if (configData.error && configData.error.includes("TLS handshake failure")) {
        // Expose a non-blocking wizard state instead of a hard blocking error that locks out the app
        setShowAtlasGuide(true);
        setSystemWarning(configData.error);
      } else {
    ...
    <div className="flex h-screen bg-zinc-50 text-zinc-900 overflow-hidden font-sans">
      ...
      <main className="flex-grow h-full overflow-y-auto flex flex-col justify-between">
        ...
        <div className="p-8 flex-grow">
          {errorString ? (
            /* RED ERROR ALERT WITH RETRY BUTTON - Hard failure */
            <div className="w-full max-w-2xl mx-auto my-6 space-y-6">
              <div className="bg-rose-50 border border-rose-100 rounded-2xl p-6 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                <h3 className="text-base font-bold text-rose-800 tracking-tight font-sans">Database Link Failure</h3>
                <p className="text-zinc-650 text-xs mt-1.5 max-w-lg leading-relaxed font-medium">
